...
def main():
    ...
    data_path = "/home/sukja/drone_Referee/DL model/drone_Referee.v2i.yolov8/data.yaml"

    print("🚀 바퀴 접지점 모델 학습을 시작합니다!")
    ...
    results = model.train(
        ...
        pose=12.0,      # 기본값 유지 (keypoint도 중요하지만 bbox랑 균형)
        kobj=2.0,       # 기본값(1.0)보다 살짝 올림 (접지점 존재 여부는 중요)
        
        # --- Data Augmentation ---
        # Roboflow에서 이미 적용: noise, blur
        # 여기서는 Roboflow가 안 해주는 것만 적용
        degrees=30.0,       # 드론 시점 회전 (360은 과함, 30 정도가 현실적)
        shear=5.0,          # 살짝만 (과하면 바퀴 형태가 너무 뒤틀림)
        ...
        mixup=0.1,          # 이미지 혼합 (소량 데이터 보강)
        
        project="wheel_runs",
        name="train_wheel_keypoint",
        save=True
    )
