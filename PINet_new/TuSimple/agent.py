        ...
        for batch_index, batch in enumerate(target_lanes):
            for lane_index, lane in enumerate(batch):
                for point_index, point in enumerate(lane):
                    if point > 0:
                        x_index = min(int(point/self.p.resize_ratio), self.p.grid_x - 1)
                        y_index = min(int(target_h[batch_index][lane_index][point_index]/self.p.resize_ratio), self.p.grid_y - 1)
