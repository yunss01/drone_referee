...
def preprocess(img_path, cfg, processes):
    ...
    ori_img = cv2.rotate(ori_img, cv2.ROTATE_90_CLOCKWISE)  # ← 추가
...
def show(model, data, cfg):
    ...
    if len(data['lanes']) == 0:
        result_img = cv2.rotate(data['ori_img'], cv2.ROTATE_90_COUNTERCLOCKWISE)
        imshow_lanes(result_img, [], show=False, out_file=out_file)
        return
    ...
    # 차선 좌표를 원본 이미지 기준으로 역변환
    # 회전: 90CW → new_x = orig_h-1-y, new_y = x
    # 역변환: orig_x = rotated_y, orig_y = 719 - rotated_x
    orig_h = 1080
    lanes_orig = []
    for lane in lanes:
        lane_orig = []
        for x, y in lane:
            ...
            orig_x = y
            orig_y = orig_h - 1 - x
            lane_orig.append((orig_x, orig_y))
        lanes_orig.append(lane_orig)
    
    result_img = cv2.rotate(data['ori_img'], cv2.ROTATE_90_COUNTERCLOCKWISE)
    imshow_lanes(result_img, lanes_orig, show=False, out_file=out_file)
